...
# Đảm bảo import được module từ thư mục gốc và thư mục src
...
from src.services.ExtractImgDBSingle import ExtractImageDBSingle

# Cấu hình UI
st.set_page_config(page_title="Tìm kiếm ảnh lá cây (Single Pass)", layout="wide")
st.title("🌿 Hệ thống Tìm kiếm Ảnh Lá Cây (Single Pass Weighted)")
st.markdown("""
Hệ thống này sử dụng phương pháp tìm kiếm **lọc một lần duy nhất** (Single Pass):
1. **Tính toán đồng thời**: Cả 4 loại đặc trưng (Shape, Texture, HOG, Color) được tính khoảng cách cùng lúc.
2. **Trọng số kết hợp**: Tổng khoảng cách được tính theo trọng số đã định trước để tìm ra **k láng giềng** giống nhất.
""")
...
if uploaded_file is not None:
    ...
    with col2:
        ...
        try:
            ...
            with st.spinner("Đang tìm kiếm k láng giềng (Single Pass)..."):
                # 3. Tìm kiếm bằng ExtractImageDBSingle
                search_engine = ExtractImageDBSingle(k_langgieng=top_k, dao_minio=dao_minio, dao_postgresql=dao_postgresql)
            ...
            if not top_results:
                ...
                st.success(f"Tìm thấy {len(top_results)} kết quả phù hợp bằng phương pháp Single Pass!")
                        ...
                        if i + j < len(top_results):
                            ...
                            with cols[j]:
                                try:
                                    ...
                                    st.warning(f"Không thể tải ảnh {img_id}")
                                    st.caption(str(minio_err))
                                    
                                st.caption(f"**Loài:** {category}")
                                st.caption(f"**Tổng khoảng cách:** {dist:.4f}")
