...
class ExtractImageDBHBT:
    ...
    def extract_image_postgresql(self, feauture_img_query: dict, debug_mode: bool = False) -> list[tuple[float, str, dict]]:
        ...
        candidate_shap_texture = []

        # K1 là số lượng ứng viên từ giai đoạn 1 (Shape + Texture)
        # K2 là số lượng kết quả cuối cùng (HOG + Color)
        K1 = min(30, self.k_langgieng * 2)
        K2 = self.k_langgieng

        for batch in self.dao_postgresql.get_features_in_batches():
            for item in batch:
                # Kiểm tra đầy đủ các đặc trưng cần thiết cho pass 1
                if (item["shape"] is not None and feauture_img_query.get("shape") is not None and 
                    item["texture"] is not None and feauture_img_query.get("texture") is not None):
                    ...
                    dis_shape_texture = distance_shape * FEATURE_WEIGHTS["shape"] + distance_texture * FEATURE_WEIGHTS["texture"]
                    ...
                    if len(candidate_shap_texture) < K1:
                        heapq.heappush(candidate_shap_texture, (-dis_shape_texture, item, debug_info))                  
                    else:
                        if dis_shape_texture < -candidate_shap_texture[0][0]:
                            heapq.heapreplace(candidate_shap_texture, (-dis_shape_texture, item, debug_info))
                
        # Giai đoạn 2: Ranking lại các ứng viên bằng HOG và Color
        for score, item, debug_info in candidate_shap_texture:
            if (item["hog"] is not None and feauture_img_query.get("hog") is not None and 
                ...
                dis_hog_color = distance_hog * FEATURE_WEIGHTS["hog"] + distance_color * FEATURE_WEIGHTS["color"]
                
                total_distance = -score + dis_hog_color
                
                if debug_mode:
                    debug_info["hog"] = distance_hog
                    debug_info["color"] = distance_color
                    debug_info["total"] = total_distance

                if len(self.k5_images) < K2:
