...
# Cấu hình UI
st.set_page_config(page_title="Tìm kiếm ảnh lá cây", layout="wide")
st.title("🌿 Hệ thống Tìm kiếm Ảnh Lá Cây (CBIR)")
...
top_k = st.sidebar.slider("Số lượng kết quả", min_value=5, max_value=50, value=10)
    ...
    with col2:
        ...
        try:
            ...
            with st.spinner("Đang tìm kiếm trong cơ sở dữ liệu..."):
                # 3. Tìm kiếm bằng ExtractImageDBWeighted (Khoảng cách Euclidean tối ưu)
                search_engine = ExtractImageDBWeighted(dao_minio, dao_postgresql)
                
                # Trả về list: [(distance, image_id), ...]
                top_results = search_engine.extract_image_postgresql(query_features_normalized, top_k=top_k)
            
            if not top_results:
                st.warning("Không tìm thấy kết quả nào. Có thể cơ sở dữ liệu trống.")
            else:
                st.success(f"Tìm thấy {len(top_results)} kết quả phù hợp!")
